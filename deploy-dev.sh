...
if [ -z $IS_GREEN_EXIST ];then
  ...
  (cd $DIR && sudo docker compose --env-file .env -f docker-compose.yml -f docker-compose.dev.yml up caddy nestjs-green spring-green swagger -d --build)
...
# blue up
  ...
  (cd $DIR && sudo docker compose --env-file .env -f docker-compose.yml -f docker-compose.dev.yml up caddy nestjs-blue spring-blue swagger -d --build)
