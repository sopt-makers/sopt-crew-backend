...
public interface InternalMeetingStatsApi {
	...
	@Operation(
		summary = "[Internal] Playground '좋아요'수 동기화",
		description = "Playground '좋아요'수를 동기화합니다. 이미 좋아요가 되어 있으면 취소하고, 없으면 추가합니다."
	)
	@ApiResponses(value = {
		@ApiResponse(responseCode = "201", description = "좋아요 수 동기화 성공", content = @Content(mediaType = "application/json")),
		@ApiResponse(responseCode = "404", description = "존재하지 않는 유저입니다.", content = @Content(mediaType = "application/json"))
	})
	ResponseEntity<InternalPostLikeResponseDto> switchPostLike(
		@Valid @RequestBody InternalPostLikeRequestDto requestBody
	);
}
